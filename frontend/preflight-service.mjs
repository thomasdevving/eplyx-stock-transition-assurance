...
export function validatePreflightFields(request){
 if(!request||typeof request!=='object'||Array.isArray(request)||Object.keys(request).some(k=>!['source','successor_mint','effective_at','deadline','post_deadline','assurance','check_ids','conversion_check_id'].includes(k))||typeof request.source!=='string'||request.source.length>44||(request.successor_mint!==null&&(typeof request.successor_mint!=='string'||request.successor_mint.length>44))||typeof request.effective_at!=='string'||request.effective_at.length>40||(request.deadline!==null&&(typeof request.deadline!=='string'||request.deadline.length>40))||![null,'TransitionStillRequired'].includes(request.post_deadline)||!['Mobility','FullTransition'].includes(request.assurance)||!Array.isArray(request.check_ids)||request.check_ids.length>4||request.check_ids.some(id=>!validRunId(id))||new Set(request.check_ids).size!==request.check_ids.length||(request.conversion_check_id!==null&&request.conversion_check_id!==undefined&&(!validRunId(request.conversion_check_id)||request.check_ids.includes(request.conversion_check_id))))throw new Error('InvalidPreflight');
 return request;
}
export async function createPreflight(service,parentId,request,key,owner){
 validatePreflightFields(request);
 request={...request,check_ids:[...request.check_ids].sort(),conversion_check_id:request.conversion_check_id??null};
 ...
 let conversion=null;
 if(request.conversion_check_id){
  const job=service.get(request.conversion_check_id);
  if(!job||job.owner!==owner||job.parent_run_id!==parentId||job.status!=='Completed'||!job.conversion_request||job.conversion_request.source!==request.source||job.parent_capture_sha256!==parent.capture_sha256||(request.successor_mint&&job.conversion_request.replacement_mint!==request.successor_mint))throw new Error('InvalidCheckSelection');
  await service.artifact(job.id);
  conversion={id:job.id,capture_sha256:job.capture_sha256,result_sha256:job.canonical_sha256,engine_sha256:job.engine_sha256,plan_sha256:job.plan_sha256,program_sha256:job.candidate_program_sha256,capture:(await service.artifact(job.id,true)).toString()};
 }
 ...
 const input={run_id:parentId,preflight_id:id,created_at,engine_sha256:hash(await readFile(service.executable)),wallet_capture:wallet.toString(),wallet_sha256:parent.capture_sha256,request,successor_capture:successorCapture,checks,conversion};
...
export async function executePreflight(service,job){
 ...
 if(code!==0||result.kind!=='current-preflight'||result.run_id!==parent.id||result.preflight_id!==job.id||result.wallet_capture_sha256!==parent.capture_sha256||result.scenario_sha256!==job.scenario_sha256||result.bundle_sha256!==job.capture_sha256||result.engine_sha256!==job.engine_sha256||result.entity.account!==job.preflight_request.source||result.entity.mint!==parent.selection.mint||result.proposed_change.source!=='UserProposed'||result.replacement_conversion?.official_transition!=='NotTested'||result.replacement_conversion?.issuer_binding_established!==false||(job.preflight_request.conversion_check_id?result.replacement_conversion?.check_id!==job.preflight_request.conversion_check_id:result.replacement_conversion?.status!=='NotTested')||result.authorization!==false||result.funds_moved!==false||result.population_readiness!==null)throw new Error('InvalidEngineResult');
