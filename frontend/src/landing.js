...
const transitionTerms = ['transition', 'stock split', 'merger', 'share conversion', 'token migration', 'delisting', 'IPO', 'acquisition'];
const flow = {
 overview: [
  ...
  ['04', 'Test and search', 'Eplyx tests selected production accounts. If a case fails, bounded search checks additional accounts and amount or reserve variants.'],
 ...
 technical: [
  ...
  ['04', 'Stress & search', 'Eplyx rechecks the frozen account selection against final state, then searches observed accounts and typed amount or reserve variants within fixed limits.'],
...
const capabilities = [
 ['01', 'Candidate conversion check', 'Do the tested balance changes reconcile?', 'Eplyx reports Proven only after the packaged candidate runs in LiteSVM and the source debit, burn, fee, ratio, rounding, reserve and replacement credit reconcile.', 'LiteSVM · exact reconciliation'],
 ['02', 'Stress on production accounts', 'Selected accounts at final capture', 'Eplyx freezes a bounded selection of observed accounts, including large balances and distinct account shapes and balance ranges. Each selected account runs against its own final captured state.', 'Frozen selection · final-state rebinding'],
 ['03', 'Counterexample search', 'Which tested value first fails?', 'For a failed case, bounded search checks additional captured accounts and typed amount or reserve variants. The report records any boundary the search finds.', 'Observed and derived · minimized'],
 ...
 ['06', 'Checks for one holder account', 'Which actions passed for this account?', 'Inspect a token or public wallet, then run a local transfer, market exit or candidate conversion check for one selected account. No wallet connection is required.', 'Read-only · VM · no signing'],
];
const trust = [
...
function exampleRun() {
 ...
 <div class="run-pair reveal">
  <article class="run-window"><div class="window-bar"><span><i></i><i></i><i></i></span><b>$ eplyx preflight · reserve funded</b><em class="run-verdict run-verdict--warn">Pass with warnings</em></div><dl>
   ...
   ${run('Counterexample search', `0 counterexamples in ${h.searchExecutions} additional executions`, 'pos')}
  </dl><p class="run-note">All ten selected stress cases passed. Stress and population readiness remain Incomplete, and authority inspection was bounded. The block-only gate passed with warnings.</p></article>
...
export function LandingPage({ analysis = false } = {}) {
 ...
 <section class="hero">${Header()}
  ...
  <div class="hero__copy reveal"><p class="eyebrow"><span></span> Transition assurance for tokenized stocks on Solana</p><h1>Rehearse the<span class="visually-hidden"> transition </span><em class="transition-roll" aria-hidden="true"><span class="transition-roll__track">${[...transitionTerms, transitionTerms[0]].map(term => `<span class="transition-roll__term">${term}</span>`).join('')}</span></em>before it goes live.</h1><p class="hero__lead">Eplyx tests a candidate stock transition against real Solana production state before deployment, showing exactly where the rollout works, where it breaks, and why.</p><div class="hero__actions"><a href="#start" class="button button--primary">Use Eplyx <span>↓</span></a><a href="#how" class="button button--text">How it works <span>↓</span></a></div></div>
 ...
 <section class="proofs section section--ink" id="product"><div class="section-heading section-heading--light reveal"><p class="eyebrow"><span></span> Capabilities</p><h2>What Eplyx<br>checks.</h2><p>Each result identifies the captured state and exact scope it covers.</p></div><div class="proof-grid proof-grid--three reveal">${capabilities.map(([n,t,q,b,s])=>`<article><div><span>${n} / ${t}</span><small class="technical-only">${s}</small></div><h3>${q}</h3><p>${b}</p></article>`).join('')}</div><div class="team-strip reveal"><div><h3>Work as a team</h3><p>The local dashboard shows runs saved on your machine. Optional sync brings local and CI results into a private workspace for history, counterexamples and comparisons. CLI analysis runs on your machine or in CI.</p></div>${workspaceLink('Open the team workspace <span>↗</span>', 'button button--light')}</div></section>
 ${exampleRun()}
 <section class="result section" id="result"><div class="section-heading reveal"><p class="eyebrow eyebrow--dark"><span></span> Holder impact · saved example</p><h2>What the saved example<br>shows for holders.</h2><div class="section-heading__aside"><p>The published SpaceX PreStocks example records which actions passed for selected holdings and which still need evidence. Open the evidence to see every check and its limits, or run an analysis on current data.</p><div class="section-heading__actions"><a href="/evidence" data-link class="button button--ink">Explore evidence <span>↗</span></a><a href="/analysis#analysis" data-link class="button button--text button--dark">Run an analysis <span>↗</span></a></div></div></div>
 ...
 <p>Saved local tests show a transfer and market exit for one account, plus principal withdrawal from a separate liquidity position. Official conversion remains untested, and most positive-balance accounts have no matching evidence.</p>
